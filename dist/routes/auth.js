    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            ...
            case 3:
                ...
                if (emailUser)
                    errors.email = "该邮箱已注册，请登录";
                if (usernameUser)
                    errors.username = "该用户名已被使用";
                if (Object.keys(errors).length > 0) {
                    return [2, res.status(400).json(errors)];
                }
                user_2 = new User_1.default({ email: email, username: username, password: password });
                return [4, class_validator_1.validate(user_2)];
            case 4:
                errors = _b.sent();
                if (errors.length > 0) {
                    return [2, res.status(400).json(mapErrors(errors))];
                }
                return [4, user_2.save()];
            case 5:
                _b.sent();
                return [2, res.json(user_2)];
            case 6:
                err_1 = _b.sent();
                console.log(err_1);
    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            ...
            case 2:
                user_3 = _b.sent();
                if (!user_3)
                    return [2, res.status(404).json({ username: "未找到用户" })];
                return [4, bcryptjs_1.default.compare(password, user_3.password)];
            case 3:
                passwordMatches = _b.sent();
                if (!passwordMatches) {
                    return [2, res.status(401).json({ password: "密码错误" })];
                }
                token = jsonwebtoken_1.default.sign({ username: username }, process.env.JWT_SECRET);
                res.set("Set-Cookie", cookie_1.default.serialize("redit", token, {
                    httpOnly: true,
...
var logout = function (_, res) {
    res.set("Set-Cookie", cookie_1.default.serialize("redit", "", {
        httpOnly: true,
        secure: process.env.NODE_ENV === "production",
        sameSite: "strict",
        expires: new Date(0),
