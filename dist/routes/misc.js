        ...
        switch (_b.label) {
            case 0:
                ...
                if (![-1, 0, 1].includes(value)) {
                    return [2, res.status(400).json({ value: "Value must be -1,0 or 1" })];
                }
                _b.label = 1;
            case 1:
                _b.trys.push([1, 16, , 17]);
                user_2 = res.locals.user;
                return [4, Post_1.default.findOneOrFail({ identifier: identifier, slug: slug })];
            case 2:
                post = _b.sent();
                comment = void 0;
                if (!commentIdentifier) return [3, 5];
                return [4, Comment_1.default.findOneOrFail({ identifier: commentIdentifier })];
            case 3:
                comment = _b.sent();
                return [4, Vote_1.default.findOne({ user: user_2, comment: comment })];
            case 4:
                vote_1 = _b.sent();
                return [3, 7];
            case 5: return [4, Vote_1.default.findOne({ user: user_2, post: post })];
            case 6:
                vote_1 = _b.sent();
                _b.label = 7;
            case 7:
                if (!(!vote_1 && value === 0)) return [3, 8];
                return [2, res.status(404).json("未找到投票")];
            case 8:
                if (!!vote_1) return [3, 10];
                vote_1 = new Vote_1.default({ user: user_2, value: value });
                if (comment)
                    vote_1.comment = comment;
                else
                    vote_1.post = post;
                return [4, vote_1.save()];
            case 9:
                _b.sent();
                return [3, 14];
            case 10:
                if (!(value === 0)) return [3, 12];
                return [4, vote_1.remove()];
            case 11:
                _b.sent();
                return [3, 14];
            case 12:
                if (!(vote_1.value !== value)) return [3, 14];
                vote_1.value = value;
                return [4, vote_1.save()];
            case 13:
                _b.sent();
                _b.label = 14;
            case 14: return [4, Post_1.default.findOneOrFail({ identifier: identifier, slug: slug }, { relations: ["comments", "comments.votes", "sub", "votes"] })];
            case 15:
                post = _b.sent();
            ...
            case 16:
                err_1 = _b.sent();
                console.log(err_1);
                return [2, res.status(500).json({ error: "有什么地方出错了" })];
            case 17: return [2];
        }
    });
}); };
var topSubs = function (_, res) { return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                imageUrlExp = "COALESCE('" + process.env.BASE_URL + "/images/'||s.\"imageUrn\",\n    'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y')";
